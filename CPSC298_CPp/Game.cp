...
void Game::play()
{
	...
	while ((!bSunk) && (this->m_nTorpedoes > 0))
	{
		...
		if (this->isOnTarget(iRow, iColumn)/* ?? call a member function to check if Battle Boat has been hit; function returs a bool ?? */)
		{
			this->showSunk();
		...
		// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
		if (this->m_nTorpedoes == 0)/* ?? number of remaining torpedoes is 0 and Battle Boat is not sunk ?? */
		{
